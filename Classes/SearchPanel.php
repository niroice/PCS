<?php
    ...
    class SearchPanel 
    {
        ...
        {  
            ...
            echo "<form action=\"CustomerClaim.php\" method=\"post\" onsubmit=\"return validateForm();\" name=\"submit-repair-claim\">";
        ...
        {   
            ...
            $this->claimType == "repair-no-transaction")
            {
                ...
                $newHiddenInputsPanel = new HiddenInputsPanel($this->claimType, $this->costPrice);

            }
            
            // financial claim show information panel and set form to financial claim
            else if($this->claimType == "financial-warranty" || $this->claimType == "financial-outside-warranty" || 
               $this->claimType == "financial-no-transaction")
            {
                // show the information panel
                $newInformationPanel = new InformationPanel($this->keycodeResult, $this->transactionResult);
        ...
        {   
            ...
            {
                $result =  mysql_query("SELECT Keycode, Claimable, Repairable,                                                     WarrantyMonths, CostPrice
                                        FROM Product 
                                        WHERE Product.Keycode = $this->keycode");

                $array = mysql_fetch_array($result);
                
                $this->costPrice = $array['CostPrice'];
                ...
                { 
                 ...
                 // if product offers a financial claim
                else if ($array["Claimable"] == 1 && $array["Repairable"] == 0)
                {
                    ...
                    if (!empty($this->transactionResult))
                    {
                        ...
                        {
                            $this->claimType = "financial-warranty";
                        }
                        else // false means outside warranty period
                        {
                            $this->claimType = "financial-outside-warranty";
                        }
                    }
